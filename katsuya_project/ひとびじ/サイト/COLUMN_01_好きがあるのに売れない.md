---
title: 「好き」があるのに売れないのは、才能がないからじゃない
project: ひとびじ
type: writing
status: draft
tags:
...
|------|------|
| 日付 | 2026.08.11 |
| タイトル | 「好き」があるのに売れないのは、才能がないからじゃない |
| 一言リード | 好きや経験はある。足りないのは努力ではなく、「届ける順番」だった——という再定義のコラム。 |
...
なぜなら、詰まりやすいのは「能力」ではなく、**届け方の順番**だからです。
...
コンセプトは、あなたの得意技の名前ではなく、**相手の人生のどこを動かすかの一言**です。
...
1. **一言で言うと、誰のどんな状態を動かす仕事か。**  
　（メソッド名ではなく、相手の言葉で言えるか）

2. **興味を持った人が、次に進む場所は一箇所に決まっているか。**  
　（SNS・LP・無料・相談の道が枝分かれしていないか）

3. **説明するとき、最初の5分は「商品」ではなく「相手の物語」から入れているか。**  
　（スペックの前に、意味の書き換えがあるか）
...
2. タイトルに、上記「投稿タイトル」を貼る
3. 本文エリアに、`## うまくいかない理由は…` から `ご相談は、サイトのCONTACTから…` までを貼る  
   （冒頭のフック段落「いいものを持っているのに…」〜「それは違うと思っています。」も含める）
4. 抜粋欄がある場合は、一覧用メタの「一言リード」を入れる（任意）
5. **公開**する  
   → トップページの COLUMN に、日付＋タイトルで自動表示される

### 注意

- 見出し（`##` `###`）は、エディタがブロック変換する場合はそのままでOK
- 一覧用メタの表や「WordPress投稿用」見出しは**公開本文に含めない**
- 日付は公開日に合わせてWordPress側で自動表示される（トップは投稿の公開日）
