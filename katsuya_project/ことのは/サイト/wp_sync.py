#!/usr/bin/env python3
"""Cursor の Markdown と kotonoha.co の投稿／固定ページを同期する。
...
def cmd_setup(_args: argparse.Namespace) -> None:
    ...
    url = input(f"サイトURL [{current.get('WP_URL', 'https://kotonoha.co')}]: ").strip()
    user = input(f"ユーザー名 [{current.get('WP_USER', '')}]: ").strip()
    password = input("アプリケーションパスワード: ").strip().replace(" ", "")
    url = url or current.get("WP_URL") or "https://kotonoha.co"
