...
def build_pdf():
    ...
    table_row_3(pdf, col_5w, ["④ 教える先生（オンライン）", "ストアカなど", "1講座 10〜15万円"])
    table_row_3(pdf, col_5w, ["⑤ 教える先生（対面）", "カルチャーセンター", "1講座 10〜15万円"])
    pdf.ln(6)
    ...
    check(pdf, "育成したプロ占い師150名以上の実例から組まれた再現性メソッド")
    ...
    body_indent(pdf, "累計1,000人以上に教えてきた、再現性100%の内容です。")
    pdf.ln(2)
    ...
    pdf.ln(2)
    body(pdf, "ストアカでの実績は累計1,614人。講座賞も受賞済みです。")
    pdf.ln(6)
    ...
    table_row_3_b(pdf, col_pay, ["銀行振込（一括）", "300,000円", "一括"], bold_col=1)
    table_row_3(pdf, col_pay, ["クレジットカード（一括）", "330,000円", "一括"])
    table_row_3(pdf, col_pay, ["銀行振込（2回払い）", "310,000円", "① 155,000円 ／ ② 155,000円"])
    table_row_3(pdf, col_pay, ["銀行振込（3回払い）", "320,000円", "① 120,000 ／ ② 100,000 ／ ③ 100,000"])
    table_row_3(pdf, col_pay, ["PayPal（10回分割）", "343,000円", "① 入学金 100,000 ／ ②〜⑩ 27,000"])
    pdf.ln(5)
    ...
    bold_line(pdf, "銀行振込一括が、もっともお得なお支払い方法です。")
    pdf.ln(6)
    ...
    table_row_3(pdf, col_sim, ["鑑定書（5,000円）月10件", "約 5万円", "約 4ヶ月"])
    table_row_3(pdf, col_sim, ["オンライン鑑定（8,000円）月10件", "約 8万円", "約 2.5ヶ月"])
    table_row_3(pdf, col_sim, ["対面鑑定 週2日（横浜中華街モデルの半分）", "約 10万円", "約 2ヶ月"])
    pdf.ln(5)

    step(pdf, "手相の先生コース（300,000円）の場合")
    pdf.ln(2)
    table_header_3(pdf, col_sim, ["活動例", "1ヶ月の収入", "回収までの目安"])
    table_row_3(pdf, col_sim, ["上記＋ストアカ講座 月1回（10万円）", "約 15〜20万円", "約 2ヶ月"])
    table_row_3(pdf, col_sim, ["上記＋ストアカ講座 月2回（10万円×2）", "約 25〜30万円", "約 1.5ヶ月"])
    pdf.ln(6)
